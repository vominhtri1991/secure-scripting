...
arp_scanning()
{
	...
	while read -r subnet; do
        	...
        	if ! echo "$HOST_KNOWN" | grep -qi "$mac"; then
            		echo "New device  connect to network $subnet: IP $ip, MAC $mac"
            		echo "$ip $mac" >> $NEW_DEVICE
        	fi
    		done
...
result_action()
{
	if [ -s $NEW_DEVICE ]; then
		sending_email "NEW DEVICES IS CONNECTING TO NETWORK!!!" "We scan and detect new devices alredy connected to your network. Check detail include IP and MAC address in attachment file" "$NEW_DEVICE"
	fi
}
